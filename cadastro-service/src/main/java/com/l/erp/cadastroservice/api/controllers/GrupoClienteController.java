...
@RestController
@RequestMapping("/api/v1/grupos-clientes")
@RequiredArgsConstructor
public class GrupoClienteController {

    private final Logger logger = LoggerFactory.getLogger(GrupoClienteController.class);

    private final GrupoClienteService service;

    @GetMapping
    public ResponseEntity<Page<GrupoClienteDTO>> getAllGroups(Pageable pageable) {
        logger.info("Listando grupos de clientes");
        Optional<Long> tenantId = SecurityUtils.getCurrentTenantId();
        logger.info("Tenant ID: {}", tenantId);
        return ResponseEntity.ok(service.getAllGroups(tenantId.orElseThrow(() -> new RuntimeException("Tenant Id não encontrado!")), pageable));
    }
