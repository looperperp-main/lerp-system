...
public interface EnderecoRepository extends JpaRepository<Endereco, UUID> {
     ...
     * left join é obrigatório aqui: um inner join em e.estabelecimento eliminaria as linhas PF (estabelecimento_id null) mesmo quando e.pessoa.id já bate. */
    @Query("select e from Endereco e left join e.estabelecimento est where e.tenantId = :tenantId and (e.pessoa.id = :pessoaId or (est is not null and est.pessoa.id = :pessoaId))")
    List<Endereco> findAllByPessoaIdAndTenantId(@Param("pessoaId") UUID pessoaId, @Param("tenantId") Long tenantId);

    @Query("select e from Endereco e left join e.estabelecimento est where e.id = :id and e.tenantId = :tenantId and (e.pessoa.id = :pessoaId or (est is not null and est.pessoa.id = :pessoaId))")
