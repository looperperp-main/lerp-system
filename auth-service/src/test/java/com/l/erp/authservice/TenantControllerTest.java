...
class TenantControllerTest {
    ...
    @Test
    @WithMockUser(authorities = "TENANT_UPDATE")
    void shouldntUpdateTenantDuplicated() throws Exception {
        var tokenAttrName = "_csrf";
        var httpSessionCsrfTokenRepository = new HttpSessionCsrfTokenRepository();
        var csrfToken = httpSessionCsrfTokenRepository.generateToken(new MockHttpServletRequest());
        ...
        when(tenantRepository.findById(1L)).thenReturn(Optional.of(oldTenant));
        when(authMapper.toTenant(any())).thenReturn(oldTenant);
        when(tenantRepository.save(any(Tenant.class))).thenThrow(DataIntegrityViolationException.class);
    ...
    void updateTenantStatusByIdErrorCancelado() throws Exception {
        var tokenAttrName = "_csrf";
        var httpSessionCsrfTokenRepository = new HttpSessionCsrfTokenRepository();
        var csrfToken = httpSessionCsrfTokenRepository.generateToken(new MockHttpServletRequest());
    ...
    @Test
    @WithMockUser(authorities = "TENANT_DELETE")
    void updateTenantStatusByIdSuccess() throws Exception {
        var tokenAttrName = "_csrf";
        var httpSessionCsrfTokenRepository = new HttpSessionCsrfTokenRepository();
        var csrfToken = httpSessionCsrfTokenRepository.generateToken(new MockHttpServletRequest());
