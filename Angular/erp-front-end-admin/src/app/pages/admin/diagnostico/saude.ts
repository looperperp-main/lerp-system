    ...
    <div class="p-2 lg:p-6 bg-[#FDFBF7] min-h-screen font-sans text-gray-800">
      <div class="flex items-center justify-between mb-6">
        <div>
          ...
          <p class="text-gray-500 text-sm mt-1">Status e instâncias de cada serviço (via Eureka + /actuator/health).</p>
        </div>
        <button pButton type="button" icon="pi pi-refresh" label="Atualizar"
                class="p-button-sm p-button-outlined" [disabled]="loading()" (click)="carregar()"></button>
      </div>
      ...
      <div class="bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
        <p-table [value]="services()" [loading]="loading()" styleClass="p-datatable-sm text-sm">
          ...
          <ng-template pTemplate="emptymessage">
            <tr><td colspan="3" class="text-center py-6 text-gray-500">Nenhum serviço registrado.</td></tr>
          </ng-template>
        </p-table>
      </div>
    </div>
