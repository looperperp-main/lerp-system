...
export class PessoaForm implements OnInit {
  ...
  ngOnInit() {
    ...
    // cMun é obrigatório no XML da NF-e pro emitente (Estabelecimento) — endereço de Pessoa PF
    // direto continua opcional (spec/modulos/emissao-fiscal/emissao-fiscal.md §10). Reaproveita o
    // valor de 'tipo' do formPessoa em vez de duplicar a decisão PF/PJ no form de endereço.
    this.formPessoa
      .get('tipo')
      ?.valueChanges.subscribe((tipo) => this.atualizarValidadorIbge(tipo));
    this.atualizarValidadorIbge(this.formPessoa.get('tipo')?.value);
  ...
  private atualizarValidadorIbge(tipo: string | null | undefined) {
    const control = this.formEndereco.get('ibgeCodigo');
    if (!control) return;
    control.setValidators(tipo === 'PJ' ? [Validators.required] : []);
  ...
  isEnderecoFieldInvalid(field: string): boolean {
    const control = this.formEndereco.get(field);
    return !!(control && control.invalid && (control.dirty || control.touched));
  }
        ...
        next: (dados) => {
          if (dados && !dados.erro) {
            this.formEndereco.patchValue(
              ...
              { emitEvent: false },
            ); // Não dispara valueChanges ao preencher

            // Desabilita os campos preenchidos automaticamente (se houver valor retornado)
            if (dados.logradouro)
              this.formEndereco.get('logradouro')?.disable({ emitEvent: false });
