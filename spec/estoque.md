...
| **E6** | ✅ **FEITO, não testado (7/9/2026).** **Badge "abaixo do mínimo"** — `EstoqueConfigInternoController`+`EstoqueConfigRefDTO`+`ProdutoEstoqueConfigRepository.buscarPorProdutosEDeposito` novos no `cadastro-service` (`GET /api/v1/interno/estoque-config?produtoIds=…&depositoId=…`, atrás do `InternalRequestFilter`; `EstoqueConfigInternoControllerTest` cobre 200 e 401 sem tenant) + `CadastroServiceClient.buscarEstoqueConfig(...)` novo no `operacoes-service` + `EstoqueController.saldos()` agrupa a página por `depositoId` e enriquece `estoqueMinimo`/`abaixoMinimo` (best-effort: falha no cadastro-service vira log + mapa vazio, nunca derruba a página; `EstoqueControllerTest` ganhou `saldosDevePreencherBadgeAbaixoDoMinimo`). **Única fase que toca outro serviço** | E5 |
| **E7** | ✅ **FEITO, não testado (7/9/2026).** **Frontend `erp-front-end-web`** — itens novos no grupo de menu já existente **"Estoque & Produtos"** (não um grupo novo): (a) `saldos.ts`/`.html`/`.scss` — filtro produto/depósito, badge da E6 (span `jb-status ok`/`bad`), ação "Ajustar" abrindo modal; (b) `movimentos.ts`/`.html`/`.scss` — filtros produto/depósito/período/tipo/origem, link `routerLink` de volta ao pedido **só quando `origemTipo === 'PEDIDO_VENDA'`** (não há tela de detalhe de `RECEBIMENTO` ainda no frontend — outras origens mostram o rótulo sem link); (c) `ajuste-form.ts`/`.html`/`.scss` (saldo contado + motivo, dentro do `p-dialog` da tela de saldos). `estoque.model.ts`/`estoque.service.ts` novos. Segue `jb-*`, `{{ }}` sempre, sem `<p-toast>` próprio (usa o global do `app.html`). Rotas `estoque/saldos`/`estoque/movimentos` adicionadas ao `app.routes.ts` **sem guard de permissão** — nenhuma rota do app tem guard por permissão granular hoje (só `authGuard` no nível `/web`), então não foi criada infraestrutura nova só para esta tela; botões de ação continuam condicionados a autoridade apenas no backend (`@PreAuthorize`). **Conclui o pré-requisito para ligar a flag do §6.1** | E5 (E6 para o badge) |
