...
| **E** | **Estoque — fatia própria, antes do P2P.** Migração `estoque-schema-001` (`movimento_estoque` append-only + `estoque_saldo`, modelagem já escrita na seção "`movimento_estoque` + `estoque_saldo`") + serviço de movimento/saldo + endpoints de consulta + fecha os dois hooks pendentes do O2C: baixa `SAIDA_VENDA` em `PedidoService.expedir()` (`:265`) e estorno `ESTORNO_SAIDA_VENDA` no cancelamento (`:337`), só item `MERCADORIA`. **Ordem decidida em 06/09/2026: O2C → estoque → P2P** (antes era O2C → P2P → estoque). Motivo: o estoque é a espinha compartilhada dos dois módulos — mantê-lo dentro da Fase 3 deixa a expedição do O2C sem baixa real por mais duas fases sem ganho nenhum, e adiantá-lo faz a Fase 3 virar só o plug de `ENTRADA_COMPRA` numa máquina que já existe | Fase 0 · migração `Produto.tipo`/`codigo_servico` no `cadastro-service` (compartilhada com `o2c-vendas.md` Fase 4b) — sem ela não dá pra distinguir item de serviço |
