...
4. **API** — DTOs, `PedidoMapper` (MapStruct), `PedidoController` (10 endpoints), permissões, OpenAPI, `@WebMvcTest`.
   **🟡 Em andamento, não testado (3 de setembro de 2026).** Feito: DTOs de request/response,
...
4b. **Serviços** **[Rev. 9] — D1/D2 implementados, não testado via `mvn`; D4 ainda só spec.** Feito: `Produto.tipo`/`codigo_servico` no `cadastro-service` (D1, com `ncm` obrigatório só pra `MERCADORIA`, validado no service); `PedidoItem.tipo_item` snapshot resolvido em `PedidoController.resolverTiposDosItens` via `CadastroServiceClient.buscarProduto` e validado (guard de tipo nulo) em `PedidoService.resolverPrecoEValidarItem` (D2); transição direta `CONFIRMADO → FATURADO` pra pedido só-serviço e bloqueio 400 (`PEDIDO_EXPEDICAO_SO_MERCADORIA`) de `expedir()` nesse caso, via helper `somenteServicos()` (D2); testes escritos em `PedidoServiceTest`/`PedidoControllerTest` cobrindo os 4 casos abaixo. Faltando (D4, só spec ainda): migração `vendas-schema-002` (colunas fiscais de `pedido`); chamada a `POST /fiscal/calcular` por item no `faturar()`; parcelas calculadas sobre `valor_total_nf` em vez de `valor_total`. Testes escritos: pedido só-serviço fatura direto (sem passar por `EXPEDIDO`); pedido misto lança 400 se faturado direto de `CONFIRMADO` (segue exigindo `EXPEDIDO`); `expedir()` em pedido só-serviço retorna 400; `criarOrcamento` com item sem tipo resolvido lança 400.
