...
- **`erp-front-end-web`** (único afetado) — novo grupo de menu "Compras":
  1. **✅ [Rev. 14] Requisições** — lista + form (itens com autocomplete de produto), botões de transição por status. Implementado, **não testado**.
  2. **Cotações** — form de convite (multi-select de fornecedores ativos), tela de digitação de resposta por fornecedor e **mapa comparativo** (matriz item × fornecedor, menor preço destacado) com ação "definir vencedor". **Único item pendente da Fase 6.**
  3. **✅ [Rev. 14] Pedidos** — lista com filtros, form, timeline de status (`compra_status_historico`), aba de recebimentos embutida no detalhe. Implementado, **não testado**. **Gap:** o pré-preenchimento de `preco_unitario` com `ProdutoFornecedor.preco_custo` e de previsão de entrega com `lead_time_dias` **não foi feito** — não existe endpoint público de `ProdutoFornecedor` (só `ProdutoFornecedorInternoController`, interno, e o mock HATEOAS de `ProdutoController.findFornecedores()`); o campo fica de digitação manual até esse endpoint existir.
  4. **✅ [Rev. 14] Recebimento** — a partir do pedido: grid de itens com pendente × recebido, dados da NF, ações confirmar/faturar. Implementado, **não testado**.
...
| **6** | **Em andamento, não testado.** **[Rev. 14 — 13/09/2026]** Requisições, Pedidos (com aba de recebimento embutida) e Recebimento implementados em `erp-front-end-web`; falta só **Cotações** (form de convite, resposta por fornecedor, mapa comparativo, definir vencedor) | backend correspondente |
