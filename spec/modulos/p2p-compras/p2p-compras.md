...
**Status:** EM IMPLEMENTAÇÃO — Fase 1 (requisição de compra) feita e **testada** (`mvn test` verde + migração Liquibase aplicada com sucesso); Fase 5 (cotação multi-fornecedor) feita e **testada** (`mvn test -Dtest=CotacaoCompraServiceTest,CotacaoCompraControllerTest` verde, 39/39 — migração `compras-schema-004` aplicada com sucesso); Fase 2 (pedido de compra) feita e **compilando** (migração `compras-schema-002` aplicada com sucesso), mas seus próprios testes de service/controller ainda não foram confirmados via execução isolada nesta sessão · **Data:** 2026-07-10 · **Rev.:** 2026-07-11 (decisões do usuário aplicadas) · **Rev. 2:** 2026-07-11 (arquitetura consolidada) · **Rev. 3:** 2026-09-01 (Fase 0 implementada — módulo `operacoes-service` criado, não testada) · **Rev. 4:** 3 de setembro de 2026 (O2C — `Pedido` de venda — já existe no mesmo `operacoes-service`; `fiscal-service` já existe para cálculo IBS/CBS/IS de **saída**, entrada segue backlog; JaCoCo do módulo em 60%, não 40%) · **Rev. 5:** 3 de setembro de 2026 (compra de SERVIÇOS: depósito opcional no `requisicao_compra`/`recebimento_mercadoria`, recebimento funciona como aceite do serviço sem gerar movimento de estoque, documento fiscal do fornecedor pode ser NFS-e com código de verificação no lugar da chave de 44 caracteres) · **Rev. 6:** 6 de setembro de 2026 (estoque virou fatia própria — Fase E — antes do P2P; ordem O2C → estoque → P2P) · **Rev. 7:** 6 de setembro de 2026 (**a modelagem de estoque saiu deste spec**: `spec/estoque.md` passa a ser a fonte autoritativa de `movimento_estoque`/`estoque_saldo`, dos enums e das regras RN-EST-*; este documento mantém só o que o lado de compras consome — ver §"`movimento_estoque` + `estoque_saldo`") · **Rev. 8:** 8 de setembro de 2026 (**Fase 1 implementada e testada**: `compras-schema-001` aplicada via `liquibase-service` + entidades/repos/DTOs/mappers, `RequisicaoCompraService` com máquina de estados completa, `CompraNumeroService` de numeração, `RequisicaoCompraController` REST, permissões `COMPRAS_*` seedadas em `auth-schema-020.yaml`, testes de service/numeração/controller — `mvn test` do `operacoes-service` verde) · **Rev. 9:** 8 de setembro de 2026 (**Fases 2 e 5 implementadas, não testadas**: `compras-schema-002` (pedido de compra) e `compras-schema-004` (cotação multi-fornecedor) + entidades/repos/DTOs/mapper/assembler + `PedidoCompraService`/`CotacaoCompraService` com máquinas de estado completas + `PedidoCompraController`/`CotacaoCompraController` REST + testes de service/controller; corrigido import errado `jakarta.persistence.ColumnDefault` → `org.hibernate.annotations.ColumnDefault` em `PedidoCompra`/`PedidoCompraItem`/`CotacaoCompraFornecedor`, que quebrava a compilação. Nada rodado via `mvn`/Liquibase — build, migração e testes ainda por rodar) · **Rev. 10:** 9 de setembro de 2026 (**Fase 5 (cotação) confirmada verde**: `./mvnw test -pl operacoes-service -Dtest=CotacaoCompraServiceTest,CotacaoCompraControllerTest` rodou 39/39 sem falha, confirmando também que o fix do import `ColumnDefault` resolveu a quebra de compilação do módulo inteiro; migrações `compras-schema-002` e `compras-schema-004` aplicadas com sucesso via `liquibase-service`. Corrigido bug de teste encontrado nessa rodada: `registrarRespostaSemAutoridadeDeveRetornar403` enviava `itens: List.of()` no corpo, disparando validação `@NotEmpty` (400) antes do `@PreAuthorize` rodar — trocado por um item válido pra checar de fato a autorização. Testes próprios de service/controller da Fase 2 (pedido) ainda não foram executados isoladamente nesta sessão, mas compilam junto) · **Rev. 11:** 12 de setembro de 2026 (**Fase 2 (pedido) confirmada verde**: `PedidoCompraServiceTest`/`PedidoCompraControllerTest` rodaram 38/38 sem falha · **Fase 3 (recebimento de mercadoria) implementada, não testada via `mvn`**: `compras-schema-003` (`recebimento_mercadoria(_item)`) + entidade/repos/DTOs/mapper/assembler + `RecebimentoMercadoriaService` — RN-P2P-05 (tolerância 5%), RN-P2P-07 (dados fiscais NFE/NFSE), RN-P2P-08 (tenant scoping), RN-P2P-11 (item de serviço não baixa estoque) — + `RecebimentoMercadoriaController` REST + integração in-process com `EstoqueService` (`ENTRADA_COMPRA`/`ESTORNO_ENTRADA_COMPRA`) + eventos Kafka `compra.recebimento.confirmado`/`cancelado` (`RecebimentoEventProducer`/`Listener`, primeiro producer do módulo `compras`) + consumer novo no `cadastro-service` (`RecebimentoCompraConfirmadoConsumer`, primeiro consumer Kafka do serviço) atualizando `ProdutoFornecedor.ultimoPrecoCompra` (`cadastro-schema-015`) + testes de service/controller escritos. Fechado também um gap deixado pela Fase 2: `PedidoCompraService.encerrarSaldo` (endpoint `/pedidos/{id}/encerrar-saldo`, reservado desde então) e `recalcularStatusAposRecebimento`, com ajuste no mapa de transições (`RECEBIDO_TOTAL -> ENVIADO`, cancelamento do único recebimento que completava o pedido). **Faturamento (`/recebimentos/{id}/faturar`, `nfe.entrada.aprovada`) fica pra Fase 4**, fora de escopo desta rodada) · **Serviços:** `operacoes-service` (**novo**, foco — também dono de O2C, ver `o2c-vendas.md`) · `cadastro-service` (validação de referências via API) · `fiscal-service` (**já existe** — motor fiscal de saída; entrada/emissão NF-e ainda não) · `liquibase-service` (migrações) · `auth-service` (seed de permissões) · `Angular/erp-front-end-web` (última fase) · integração futura com `financeiro-service` (Fin.md — ainda não implementado) · **Rev. 12:** 12 de setembro de 2026 (**Fase 3 confirmada verde**: migração `compras-schema-003`/`cadastro-schema-015` aplicada com sucesso via `liquibase-service`; `mvn test` do `operacoes-service` — `RecebimentoMercadoriaServiceTest`/`RecebimentoMercadoriaControllerTest` + os testes novos de `PedidoCompraServiceTest`/`PedidoCompraControllerTest` para `encerrarSaldo`/`recalcularStatusAposRecebimento` — passou sem falha) · **Rev. 13:** 12 de setembro de 2026 (**Fase 4 (faturamento) implementada e confirmada verde**: `./mvnw test -pl operacoes-service "-Dtest=RecebimentoMercadoriaServiceTest,RecebimentoMercadoriaControllerTest,PedidoCompraServiceTest,PedidoCompraControllerTest"` rodou sem falha — `RecebimentoMercadoriaService.faturar()` — transição `CONFIRMADO -> FATURADO`, RN-P2P-06 (soma dos percentuais das parcelas = 100%, mesmo algoritmo de arredondamento de `PedidoService.calcularParcelas`, vendas), endpoint `POST /recebimentos/{id}/faturar` (`COMPRAS_FATURAR`, permissão já seedada em `auth-schema-020.yaml` desde a Fase 1) + evento `RecebimentoFaturadoEvent`/`RecebimentoEventListener` (AFTER_COMMIT) publicando `nfe.entrada.aprovada` com o payload exato do Fin.md §F4.2 (`RecebimentoEventProducer.publicarFaturado`, fornecedor/CNPJ/IBGE-destino enriquecidos via `CadastroServiceClient`, best-effort) + `PedidoCompraService.encerrarSeTodosRecebimentosFaturados` (novo método, chamado só por `faturar()`: pedido `RECEBIDO_TOTAL` com todos os recebimentos não cancelados `FATURADO` vai automaticamente a `ENCERRADO`) + `Constants.NFE_ENTRADA_APROVADA_TOPIC`/`AUDIT_ACAO_RECEBIMENTO_FATURADO`/`RECEBIMENTO_COMPRA_PARCELAS_PERCENTUAL_INVALIDO` + `CadastroServiceClient.buscarPessoa` (novo, CNPJ do fornecedor) + testes de service/controller escritos cobrindo RN-P2P-06 e a transição — campo `fornecedor_regime` sai `null` no payload porque regime tributário ainda não existe como campo no domínio `Pessoa` do cadastro-service, mesma decisão MVP de `impostos` zerados) · **Rev. 14:** 13 de setembro de 2026 (**Fase 6 (frontend) em andamento, não testada**: telas de Requisições, Pedidos de Compra e Recebimento de Mercadoria implementadas em `erp-front-end-web` (`pages/compras/requisicoes`, `pages/compras/pedidos` com o recebimento embutido no detalhe do pedido, `pages/compras/recebimentos` como model/service de apoio), rotas e menu "Compras" atualizados; gap encontrado: pré-preenchimento de `preco_unitario`/`lead_time_dias` via `ProdutoFornecedor` no form de pedido **não foi feito** — não existe endpoint público (só `ProdutoFornecedorInternoController`, interno, e o mock HATEOAS de `ProdutoController.findFornecedores()`), então o campo fica de digitação manual por ora. Falta só a tela de **Cotações** para fechar a Fase 6) · **Rev. 15:** 14 de setembro de 2026 (**Fase 6 100% implementada e testada E2E**: roteiro completo Requisição (criar→aprovar) → Cotação (convite multi-fornecedor→respostas→mapa comparativo→encerrar definindo vencedor) → Pedido (gerado automático→aprovar→enviar) → Recebimento (parcial→confirmar→faturar→pedido `ENCERRADO`) rodado ao vivo pelo usuário no navegador. Bugs encontrados durante o teste e corrigidos na mesma sessão, **ainda não re-testados após o fix**: (1) `PedidoCompraItem.quantidadeRecebida` nunca era populada na criação (manual ou via `CotacaoCompraService.encerrar()`), causando `ConstraintViolationException`/500 ao encerrar qualquer cotação — corrigido em `PedidoCompraService.criar()`; (2) tela de Requisições não tinha ação de aprovar/reprovar nem de visualizar uma requisição fora de `RASCUNHO` (só editar, desabilitado) — adicionados botões Aprovar/Reprovar/Ver detalhe (modo readonly) em `requisicoes.ts/html` e `requisicao-form.ts/html`; (3) form de "Novo Pedido" não pré-preenchia depósito/itens ao selecionar a "Requisição de Origem" — implementado em `pedido-form.ts` (compras); (4) listagem de Cotações sempre mostrava "0" em Fornecedores Convidados (assembler de listagem não populava a relação, só o detalhe) — corrigido com contagem agregada em lote (`CotacaoCompraFornecedorRepository.countByCotacaoIdIn` + `CotacaoCompraService`/`Controller`, novo campo `quantidadeFornecedoresConvidados` no DTO); (5) no form de Cotação, "Enviar Convite" não fazia nada ao selecionar Requisição de Origem — o `FormArray` de itens ficava escondido via `*ngIf` mas continuava habilitado e inválido, travando `form.invalid` silenciosamente — corrigido desabilitando o array quando há requisição selecionada (`cotacao-form.ts`); (6) card de Recebimentos no detalhe do pedido não mostrava os itens — mesma causa do bug (4), listagem de recebimentos não populava itens — corrigido com busca em lote (`RecebimentoMercadoriaItemRepository.findAllByRecebimentoIdIn` + `RecebimentoMercadoriaService`/`Controller`). Também corrigido, fora do fluxo funcional: CSS do modal "vidro" (`--glass-dialog-bg`/`--glass-dialog-border`) hardcoded pra tema escuro, ilegível no tema claro — corrigido nas 5 telas do módulo compras (`pedidos`, `pedido-detalhe`, `requisicoes`, `cotacoes`, `cotacao-detalhe`); o mesmo padrão hardcoded ainda existe em outras ~21 telas do sistema, fora do escopo desta rodada. **Nada rodado via `mvn`/`ng build` pelo Claude — usuário testou tudo ao vivo no navegador; os fixes desta lista ainda precisam ser re-testados**)
...
- Estoque: **`movimento_estoque`** (append-only, imutável) + **`estoque_saldo`** (materializado por produto+depósito, atualizado na mesma transação). **[Rev. 7] A modelagem, os enums, as regras e as fases saíram deste spec — a fonte autoritativa é `spec/estoque.md`** (escrito em 6 de setembro de 2026), e o estoque virou fatia própria (**Fase E**) implementada **antes** do P2P. Aqui fica só o que o lado de compras consome. Antes disso não existia nenhuma tabela de saldo/movimento no projeto — `ProdutoEstoqueConfig` é só parametrização (mín/máx/ponto de reposição).
...
> **[Rev. 7 — 6 de setembro de 2026] A modelagem completa mora agora em `spec/estoque.md` (§3), que é a fonte autoritativa.** Este spec deixou de duplicá-la para não ter duas versões divergentes da mesma tabela. O que segue é só o resumo do que o **lado de compras** consome; qualquer coluna, enum, índice ou regra é decidido lá.

Dono: o módulo de estoque dentro do `operacoes-service`. O módulo de compras chama esse módulo direto, na mesma transação (ver "Integração com estoque") — não é evento, porque estão no mesmo serviço. O módulo de vendas (expedição, `o2c-vendas.md`) alimenta as mesmas tabelas pelo mesmo caminho in-process (`SAIDA_VENDA`).

**`movimento_estoque`** — append-only, nunca UPDATE/DELETE; estorno = movimento contrário. **`estoque_saldo`** — saldo materializado por (`tenant_id`, `produto_id`, `deposito_id`), atualizado com `SELECT ... FOR UPDATE` (upsert) na mesma transação do movimento.

O que compras usa da tabela:

| Campo | Valor no recebimento |
|---|---|
| `tipo` | `ENTRADA_COMPRA` na confirmação · `ESTORNO_ENTRADA_COMPRA` no cancelamento de recebimento CONFIRMADO |
| `origem_tipo` / `origem_id` | `RECEBIMENTO` / `recebimento_mercadoria.id` |
| `quantidade` | sempre positiva — o sinal vem do `tipo` |
| `valor_unitario` | **[Rev. 7]** coluna nova (nullable): preencher com o **preço unitário da NF de entrada**. É a matéria-prima do custo médio ponderado (ver YAGNI); numa tabela append-only, o que não for gravado na hora não pode ser reconstruído depois |
| `motivo` | **[Rev. 7]** coluna nova, nula aqui — obrigatória só em ajuste/inventário manual |

**Diferenças em relação ao desenho que estava neste spec até a Rev. 6** (todas fechadas em `spec/estoque.md`, nenhuma exige mudança no fluxo de compras):

- `origem_id` passou de `NOT NULL` para **nullable com CHECK** (`origem_tipo IN ('AJUSTE','INVENTARIO') OR origem_id IS NOT NULL`) — ajuste e inventário manual não têm documento de origem. Recebimento continua sempre preenchendo.
- `valor_unitario NUMERIC(15,4) NULL` e `motivo VARCHAR(500) NULL` são colunas novas.
- O enum de `tipo` (`VARCHAR(25)`) fechou em 6 valores: `ENTRADA_COMPRA`, `ESTORNO_ENTRADA_COMPRA`, `SAIDA_VENDA`, `ESTORNO_SAIDA_VENDA`, `AJUSTE_ENTRADA`, `AJUSTE_SAIDA`. `INVENTARIO` **não** é tipo — é valor de `origem_tipo` (um tipo `INVENTARIO` teria sinal indefinido, já que contagem física sobe ou desce).
- Índice único parcial `uq_mov_estoque_origem_produto` (`tenant_id`, `tipo`, `origem_tipo`, `origem_id`, `produto_id`) `WHERE origem_id IS NOT NULL` — idempotência por documento. **Impacto direto no recebimento:** o serviço de estoque agrega as linhas por produto antes de gravar, então um recebimento com o mesmo produto em duas linhas gera **um** movimento com a soma, não dois (ver "Integração com estoque", passo 5).
- Sem FK física para `produto`/`deposito`: as tabelas de referência estão em outro schema/serviço (`cadastros`), mesmo padrão de `vendas.pedido`.

**[Rev. 5]** Item de recebimento com `Produto.tipo = SERVICO` **não gera linha em `movimento_estoque`** — só itens `MERCADORIA` alimentam esta tabela (RN-EST-01 do `estoque.md`; ver nota em `recebimento_mercadoria_item` acima).

CHECK `quantidade >= 0` **não** é aplicado — estoque negativo sistêmico é aceitável no MVP. **[Rev. 7] A "flag temporária" citada até a Rev. 6 virou código real:** `estoque.bloquear-saida` (default `false`), lida dentro do `EstoqueService` e aplicada a toda saída, não só à expedição (RN-EST-05). O recebimento **nunca** é afetado por ela — receber só *aumenta* saldo, então "saldo insuficiente" não se aplica (RN-EST-08); a validação futura equivalente no recebimento é outra, de **capacidade física do depósito** (espaço disponível), não de saldo do produto.
...
- `estoque/estoque-schema-001.yaml` — `CREATE SCHEMA IF NOT EXISTS estoque` + `movimento_estoque`, `estoque_saldo`, CHECKs e 3 índices. **[Rev. 7] Não pertence a este spec:** é a Fase E1 de `spec/estoque.md` (§3.4), que roda **antes** do P2P. Sai também `auth/auth-schema-019.yaml` (permissões `ESTOQUE_*`), que estava listado na Fase 1 daqui. Nota: o schema `estoque` **não** existe no `init/000-initial-schemas.yaml` (que cria `auth`, `cadastros`, `vendas`, `financeiro`, `logistica`, `rh`), por isso nasce no próprio changeset, no padrão de `fiscal-001-create-schema`.
...
| POST | `/api/v1/estoque/ajustes` | **[Rev. 7]** ajuste/inventário manual por **saldo contado** (o operador informa o saldo real, o serviço calcula a diferença); permissão `ESTOQUE_AJUSTAR` |

> **[Rev. 7]** Os três endpoints são entregues nas Fases E5/E6 de `spec/estoque.md`, **antes** do P2P — não são trabalho deste spec. O cruzamento com `ProdutoEstoqueConfig` depende de um endpoint interno novo no `cadastro-service` (que hoje não tem controller para essa entidade): é a Fase E6 lá.
...
5. **Chama o módulo de estoque, na mesma transação, só para itens com `Produto.tipo = MERCADORIA` [Rev. 5]:** uma única chamada a `EstoqueService.registrarMovimento(...)` (`spec/estoque.md` §2.2) com `tipo=ENTRADA_COMPRA`, `origemTipo=RECEBIMENTO`, `origemId=recebimento_id`, o `deposito_id` do recebimento e uma linha por item de mercadoria (`produtoId`, `quantidade`, `valorUnitario` = preço unitário da NF). O serviço agrega por produto, ordena para travar sempre na mesma sequência, faz upsert em `estoque_saldo` com lock pessimista (`FOR UPDATE`) e insere o movimento. **[Rev. 7]** Duas linhas do mesmo produto no recebimento viram **um** movimento com a soma — é o que sustenta o índice único de idempotência. Item de serviço fica fora da lista (ver nota em `recebimento_mercadoria_item`). Se qualquer passo falhar, tudo é revertido junto — não há janela de inconsistência entre recebimento e estoque.
...
> Consistência: **transacional**, não mais eventual — recebimento e estoque commitam juntos ou nenhum dos dois commita. A ausência de *validação bloqueante* de saldo continua sendo uma decisão de negócio separada (RN aceita no MVP), não uma limitação técnica da integração. **[Rev. 7]** Esse "quando o controle real de disponibilidade for exigido" já tem nome e lugar: a flag `estoque.bloquear-saida` (default `false`, RN-EST-05 de `spec/estoque.md`) — e ela vale para as **saídas**; entrada de compra nunca é bloqueada por saldo.
...
- **`erp-front-end-web`** (único afetado) — novo grupo de menu "Compras":
  1. **✅ [Rev. 15] Requisições** — lista + form (itens com autocomplete de produto), botões de transição por status. Implementado e testado E2E.
  2. **✅ [Rev. 15] Cotações** — form de convite (multi-select de fornecedores ativos), tela de digitação de resposta por fornecedor e **mapa comparativo** (matriz item × fornecedor, menor preço destacado) com ação "definir vencedor". Implementado e testado E2E.
  3. **✅ [Rev. 15] Pedidos** — lista com filtros, form, timeline de status (`compra_status_historico`), aba de recebimentos embutida no detalhe. Implementado e testado E2E. **Gap:** o pré-preenchimento de `preco_unitario` com `ProdutoFornecedor.preco_custo` e de previsão de entrega com `lead_time_dias` **não foi feito** — não existe endpoint público de `ProdutoFornecedor` (só `ProdutoFornecedorInternoController`, interno, e o mock HATEOAS de `ProdutoController.findFornecedores()`); o campo fica de digitação manual até esse endpoint existir.
  4. **✅ [Rev. 15] Recebimento** — a partir do pedido: grid de itens com pendente × recebido, dados da NF, ações confirmar/faturar. Implementado e testado E2E.
  5. ~~**Estoque**~~ — **[Rev. 7] saiu do escopo deste spec:** as telas de estoque (consulta de saldos com badge "abaixo do mínimo" via `ProdutoEstoqueConfig`, extrato de movimentos e **form de ajuste/inventário**) são a **Fase E7** de `spec/estoque.md` e já estarão no ar quando o P2P começar. O menu "Estoque" é grupo próprio, irmão de "Compras", não item dentro dele.
...
| **E** | **Estoque — fatia própria, antes do P2P. [Rev. 7] Planejada em `spec/estoque.md` (6 de setembro de 2026), que passou a ser a fonte autoritativa** — este spec não descreve mais as fases dela. Resumo: **E1** schema `estoque-schema-001` + permissões (`auth-schema-019`) · **E2** domínio/repos · **E3** `EstoqueService` + flag `estoque.bloquear-saida` · **E4** fecha a issue #89 (baixa `SAIDA_VENDA` em `PedidoService.expedir()` e estorno `ESTORNO_SAIDA_VENDA` no cancelamento, só item `MERCADORIA`) · **E5** API REST (fecha a parte estoque da issue #80) · **E6** badge "abaixo do mínimo" (endpoint interno no `cadastro-service`) · **E7** frontend (saldos, extrato, ajuste). **Ordem decidida em 06/09/2026: O2C → estoque → P2P** (antes era O2C → P2P → estoque). Motivo: o estoque é a espinha compartilhada dos dois módulos — mantê-lo dentro da Fase 3 deixa a expedição do O2C sem baixa real por mais duas fases sem ganho nenhum, e adiantá-lo faz a Fase 3 virar só o plug de `ENTRADA_COMPRA` numa máquina que já existe | Fase 0 · migração `Produto.tipo`/`codigo_servico` no `cadastro-service` (compartilhada com `o2c-vendas.md` Fase 4b) — sem ela não dá pra distinguir item de serviço |
| **1** | **✅ Feito e testado em 08/09/2026** — Migração `compras-schema-001` (schema + numeração, requisição, histórico) + entidades/repos/DTOs/mappers + `RequisicaoCompraService` (CRUD, máquina de estados RASCUNHO → PENDENTE_APROVACAO → APROVADA/REPROVADA, cancelar, reabrir) + `CompraNumeroService` (numeração via upsert + `SELECT … FOR UPDATE`, mesmo padrão de `PedidoNumeroService`) + `RequisicaoCompraController` REST (`/api/v1/compras/requisicoes`, CRUD + transições) + permissões `COMPRAS_*` seedadas no auth (`auth-schema-020.yaml`, `DOMINIO_ACAO`) + testes de service/numeração/controller (`@WebMvcTest`+MockMvc). `mvn test` do `operacoes-service` e a migração via `liquibase-service` rodados pelo usuário e verdes. **[Rev. 7]** As `ESTOQUE_*` (`ESTOQUE_VISUALIZAR`, `ESTOQUE_AJUSTAR`) saíram daqui — vão no `auth-schema-019.yaml` da Fase E1 | Fase 0 |
| **2** | **✅ Implementado em 08/09/2026** — Migração `compras-schema-002` aplicada com sucesso (09/09/2026) + pedido de compra completo (criação avulsa/da requisição, aprovação RN-P2P-01/02/03, alerta RN-P2P-04, transições, histórico) + `PedidoCompraController` REST + testes de service/controller escritos, **ainda não executados isoladamente** | Fase 1 |
| **3** | **✅ Implementado e testado em 12/09/2026** — migração `compras-schema-003` aplicada com sucesso + `mvn test` verde + recebimento (RN-P2P-05 com tolerância 5%) + chamada in-process ao módulo de estoque já existente da Fase E — **[Rev. 7]** um `EstoqueService.registrarMovimento(...)` por recebimento (`tipo=ENTRADA_COMPRA`, `origemTipo=RECEBIMENTO`, `valorUnitario` = preço unitário da NF, que é o que alimenta o custo médio futuro), mesma transação, só item `MERCADORIA` — + evento externo `compra.recebimento.confirmado`/`cancelado` (notificação) + consumer no `cadastro-service` (`ultimo_preco_compra`, com `cadastro-schema-0XX`). **[Rev. 6 — 06/09/2026]** `estoque-schema-001` e os endpoints de consulta de estoque saíram desta fase e passaram para a Fase E; a migração `Produto.tipo`/`codigo_servico` também migrou para lá, já que é ela que permite pular o passo de estoque para item de serviço | Fase 2 · Fase E |
| **4** | **✅ Implementado e testado em 12/09/2026** — `mvn test` verde — `RecebimentoMercadoriaService.faturar()` (`CONFIRMADO -> FATURADO`) + RN-P2P-06 (soma dos percentuais das parcelas = 100%) + publicação `nfe.entrada.aprovada` (payload F4.2 com impostos zerados, AFTER_COMMIT via `RecebimentoFaturadoEvent`/`RecebimentoEventListener`/`RecebimentoEventProducer.publicarFaturado`, constantes em `common`) + `PedidoCompraService.encerrarSeTodosRecebimentosFaturados` (pedido `RECEBIDO_TOTAL` com todos os recebimentos faturados vai a `ENCERRADO`) + endpoint `POST /recebimentos/{id}/faturar` (`COMPRAS_FATURAR`) + registro em `kafka-topics.md`. Publicado desde já, sem consumidor (decisão do usuário) | Fase 3 · consumo real depende do financeiro-service (Fin.md) existir |
| **5** | **✅ Implementado e testado em 09/09/2026** — Migração `compras-schema-004` aplicada com sucesso + cotação multi-fornecedor (convite, resposta, comparativo ordenado pelo critério de desempate, vencedor **escolhido manualmente** → pedido, encerramento gera `PedidoCompra` automático e atende a requisição de origem) + `CotacaoCompraController` REST + testes de service/controller **verdes** (`mvn test -Dtest=CotacaoCompraServiceTest,CotacaoCompraControllerTest`, 39/39) | Fase 2 (não bloqueia 3/4) |
| **6** | **✅ 100% implementada e testada E2E.** **[Rev. 15 — 14/09/2026]** Requisições, Cotações, Pedidos (com aba de recebimento embutida) e Recebimento em `erp-front-end-web`; roteiro completo Requisição→Cotação→Pedido→Recebimento rodado ao vivo pelo usuário no navegador, com bugs corrigidos na mesma sessão (ver detalhe no header de status) ainda não re-testados | backend correspondente |

Testes por fase no padrão do projeto (`@WebMvcTest` + MockMvc; JaCoCo ≥ 60% — **[Rev. 4]** piso do `operacoes-service` subiu de 40% para 60% junto com o O2C, vale pro módulo inteiro, P2P incluso): máquina de estados (transições válidas/inválidas), RN-P2P-04/05 (faixa de preço e quantidade), geração de parcelas (centavos). **[Rev. 7]** Os testes do módulo de estoque em si (movimento/saldo, idempotência, concorrência, estorno) são da Fase E — `spec/estoque.md` §8; aqui basta cobrir o que é específico de compras: recebimento só-serviço **não** gera movimento, recebimento misto gera movimento só das mercadorias com `valorUnitario` = preço da NF, cancelamento de recebimento gera `ESTORNO_ENTRADA_COMPRA`, e recebimento confirmado duas vezes não duplica saldo (índice único de idempotência). **Nada é considerado funcionando até rodado — o usuário executa builds/testes.**
...
| **Devolução ao fornecedor** | fluxo fiscal próprio (NF de devolução) | novo `tipo` de movimento (`SAIDA_DEVOLUCAO`) + origem própria no enum de `spec/estoque.md` §3.2 + documento próprio; até lá, cancelamento de recebimento cobre o caso simples |
...
| **Custo médio ponderado / contabilidade de estoque** | GL é spec separado (Fin.md §11.1) | **[Rev. 7] deixou de ser promessa:** `movimento_estoque.valor_unitario` existe desde a Fase E1 e o recebimento já grava o preço da NF nela (`spec/estoque.md` D3). O custo médio vira projeção sobre a tabela append-only, sem migração de dados nem backfill |
