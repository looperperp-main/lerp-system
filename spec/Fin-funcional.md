...
| RN-AP-016 | **Devolução de adiantamento** (vale para adiantamento de fornecedor e de cliente): quando o terceiro pede de volta um saldo adiantado e não consumido, o sistema gera um título de natureza **inversa** pelo valor devolvido (adiantamento recebido de cliente → título a pagar; adiantamento pago a fornecedor → título a receber). O valor devolvido deixa de estar disponível **no momento do pedido de devolução** — não na baixa do título —, impedindo que o mesmo saldo seja usado em outra baixa enquanto a devolução está em aberto. Cancelar a devolução (antes de qualquer pagamento efetivo) devolve o valor ao saldo. |
...
| **Conferência cruzada entre pedido de compra, recebimento físico e nota fiscal, com bloqueio automático de pagamento em caso de divergência** | Depende de um futuro módulo de Compras; hoje o bloqueio de pagamento existe, mas é sempre acionado manualmente. Requisitos já capturados para essa fase: parâmetro por empresa definindo a fonte da condição de pagamento (ordem de compra ou nota do fornecedor), com alerta obrigatório ao operador em caso de divergência; e alçada de aprovação na própria ordem de compra — pagamento cuja ordem já foi aprovada pela mesma alçada nasce liberado, sem reaprovação. |
| **Adiantamento vinculado a operação com limite** | O uso de adiantamento em baixas e a devolução de adiantamento estão especificados (RN-AP-007/013/016). O modelo em que notas fiscais subsequentes consomem o saldo automaticamente, sem gerar título, com trava de emissão ao esgotar o limite acordado, depende do futuro módulo de pedidos/faturamento. |
| **API bancária direta (registro instantâneo de boleto com QR PIX) e VAN bancária** | Evolução planejada da Tesouraria: mesmos boletos e mesmas baixas, mudando apenas o canal de comunicação com o banco (API em vez de arquivo de remessa/retorno). |
| **Motor de importação de dados (migração de outros sistemas)** | Módulo transversal futuro, com especificação própria: templates de importação por área (cadastros, títulos em aberto, plano de contas, saldos) para o onboarding de clientes vindos de outros ERPs. |
