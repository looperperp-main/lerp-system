...
Pipeline stages: **Checkout → Build & Test (`mvnw clean verify` on all seven services with `-am`) → SonarQube Analysis → Quality Gate (aborts on failure, `main` only) → Docker Build & Push** (images `vitorff1234/<service>:<BUILD_NUMBER>` + `:latest` to Docker Hub). Tested + analyzed: all seven modules — `auth-service`, `cadastro-service`, `partner-service`, `billing-service`, `fiscal-service`, `gateway`, `registry` — now inherit from the root `erp-vsd` pom (not `spring-boot-starter-parent` directly) and run through `verify`/Sonar in the same reactor build, each with its own `jacoco-maven-plugin` `check` execution (`coverage.minimum` — 0.20 starting floor for `gateway`/`registry`). SonarQube Community Edition doesn't support Branch/PR Analysis, so the SonarQube Analysis and Quality Gate stages only run `when { branch 'main' }` — feature branches/PRs get Build & Test only. The Docker stage builds images straight from the jars `verify` already produced (no separate packaging step). Each of the seven modules has its own `Dockerfile` (`registry` is also a `<module>` in the root POM so `-pl registry` resolves). `liquibase-service` is still not imaged (runs migrations).
...
**Integrações externas longas são assíncronas:** qualquer chamada a serviço externo cujo tempo de resposta o ERP não controla (webservice de SEFAZ para NF-e/CT-e/NFS-e, gateway de pagamento, etc.) segue padrão assíncrono — endpoint responde `202` com um id, o desfecho chega por evento Kafka e por endpoint de consulta (`GET .../{id}`), nunca conexão síncrona presa esperando o externo. Depois de timeout, o único caminho correto é **consultar pelo identificador de negócio antes de qualquer retentativa** — retry cego duplica número/registro. Motivação: revisão de arquitetura da emissão fiscal (`spec/modulos/emissao-fiscal/emissao-fiscal.md`, 14/09/2026) apontou que tratar SEFAZ com o mesmo padrão síncrono do `fiscal-service` (que responde em milissegundos e é determinístico) seria o erro clássico dessa classe de integração — vale como regra geral do projeto, não só para emissão fiscal.
